...
def autotitle(ax,slice_description):
    ...
    input: ax - handle to the matplotlib axis object
    input: slice_description - slice description dictionary
    return: nothing
    """
    ws_name = slice_description['Name']
    ws = mtd[ws_name]
    temp_array = np.array([])
    for i in range(ws.getNumExperimentInfo()):
        temp_array = np.append(temp_array, ws.getExperimentInfo(0).run()['SampleTemp'].value)
    SampleTemp_mean = np.mean(temp_array)
    SampleTemp_std = np.std(temp_array)
    auto_title_string = ws_name + ", Ei = {0} meV, T = {1:.3}$\pm${2:.3} K ".format(ws.getExperimentInfo(0).run()['EnergyRequest'].value[0], SampleTemp_mean, SampleTemp_std)
