...
data_folder = '/SNS/CNCS/IPTS-25820/nexus/'
results_folder = '/SNS/CNCS/shared/BL5-scripts/2020A/'

runs = range(336236, 336318+1)
...
flux_res_save(results_folder+'2020A-chp2-test-1meV-', Ei_list, monitor_normalized_intensity_list, monitor_normalized_intensity_perMW_list, vi_list, detector_intensity_list, detector_FWHM_list, DD_opening_list, DD_speed_list)

np.save(results_folder+'2020A-chp2-test-1meV-chp2_delay_list.npy', chp2_delay_list)

#plt.figure()
#plt.title('detector elastic intensity')
#plt.plot(chp2_delay_list, detector_intensity_list)
#plt.show()


plt.figure()
plt.title('IPTS-25820, detector elastic FWHM, runs = ({0}, {1})'.format(np.min(runs), np.max(runs)))
plt.plot(chp2_delay_list, detector_FWHM_list, '.-',label = 'fit FWHM')
plt.ylim([0,.03])
plt.hlines(0.02, np.min(chp2_delay_list), np.max(chp2_delay_list) , colors = 'red', label = '2019 fit FWHM')
plt.vlines(18121, -1,1, colors = 'green', label = '2019 chp2 delay')
plt.xlabel('chop2 delay (microseconds)')
plt.ylabel('FWHM (meV)')
plt.legend()
plt.show()
...
#plt.plot(chp2_delay_list, monitor_normalized_intensity_list)
#plt.show()


plt.figure()
plt.title('IPTS-25820, Ei = 1 meV, runs = ({0}, {1})'.format(np.min(runs), np.max(runs)))
plt.plot(chp2_delay_list, monitor_normalized_intensity_list/ np.max(monitor_normalized_intensity_list), label = 'normalized bm3')
plt.plot(chp2_delay_list, (detector_intensity_list-np.min(detector_intensity_list[0]))/ (np.max(detector_intensity_list)   -np.min(detector_intensity_list[0])), label = 'normalized detector')
plt.vlines(18121, -1,1, colors = 'red', label = '2019 chp2 delay')
chp2_period = 1.e6/60./60.
for i in range(-10,10):
    plt.vlines(18121 + i*chp2_period, -1,1, colors = 'red', linestyles = 'dashed')
