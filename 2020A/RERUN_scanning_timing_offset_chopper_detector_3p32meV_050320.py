...
flux_res_save(results_folder+'2020A-chp2-test-3p32meV-', Ei_list, monitor_normalized_intensity_list, monitor_normalized_intensity_perMW_list, vi_list, detector_intensity_list, detector_FWHM_list, DD_opening_list, DD_speed_list)

np.save(results_folder+'2020A-chp2-test-3p32meV-chp2_delay_list.npy', chp2_delay_list)
...
plt.ylim([0,.35])
plt.hlines(0.113, np.min(chp2_delay_list), np.max(chp2_delay_list) , colors = 'red', label = '2019 fit FWHM')
plt.vlines(10316, -1,1, colors = 'green', label = '2019 chp2 delay')
...
plt.figure()
plt.title('IPTS-25820, Ei = 3.32 meV, runs = ({0}, {1})'.format(np.min(runs), np.max(runs)))
...
plt.vlines(10316, -1,1, colors = 'red', label = '2019 chp2 delay')
chp2_period = 1.e6/60./60.
for i in range(-10,10):
    plt.vlines(10316 + i*chp2_period, -1,1, colors = 'red', linestyles = 'dashed')
