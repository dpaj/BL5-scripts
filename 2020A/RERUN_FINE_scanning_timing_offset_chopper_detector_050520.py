...
runs = list(range(336711, 336805+1))
...
for idx, val in enumerate(exp_log):
    if int(val[0]) in runs:
        print(val[0].decode("utf-8"), int(val[1].decode("utf-8").split('=')[1]))
        this_chp2_delay = int(val[1].decode("utf-8").split('=')[1])
...
flux_res_save(results_folder+'2020A-chp2-test-FINE-', Ei_list, monitor_normalized_intensity_list, monitor_normalized_intensity_perMW_list, vi_list, detector_intensity_list, detector_FWHM_list, DD_opening_list, DD_speed_list)

np.save(results_folder+'2020A-chp2-test-FINE-chp2_delay_list.npy', chp2_delay_list)
...
plt.ylim([0,1.])
plt.hlines(0.72, np.min(chp2_delay_list), np.max(chp2_delay_list) , colors = 'red', label = '2019 fit FWHM')
plt.vlines(5802.75, -1,1, colors = 'green', label = '2019 chp2 delay')
...
plt.plot(chp2_delay_list, detector_intensity_list/ np.max(detector_intensity_list), label = 'normalized detector')
plt.vlines(5802.75, -1,1, colors = 'red', label = '2019 chp2 delay')
chp2_period = 1.e6/60./60.
for i in range(-5,20):
    plt.vlines(5802.75 + i*chp2_period, -1,1, colors = 'red', linestyles = 'dashed')
