...
def titleup(title):
    '''
    This Function is only available for SCANS, and it sets the title of the scan. Users have the possibility to add and monitor 
    PV values such as Energy, Motor Position, Temperature and Field. Any other note can be added using the command cmdcomment. 
    param   title: Title of the scan plus the monitored PV. NOTICE THE MONITOR PV MUST BE WRITTEN IN BACK QUOTES `...`
    
    example:       titleup("E= 12.0 meV,  T=`BL5:SE:SampleTemp` K, H=`BL5:SE:FitSam:PersistentField` Tesla")         
    '''
    cmds.append(Script('SetTextPV',("BL5:SMS:RunInfo:RunTitle",title)))
    cmds.append(Set('BL5:CS:LineLog:Add',"Add title up-date."))
    print("Add title up-date {0}".format(title))
...
def Change_phase_entry(value):
    '''
    This Function Changes the Phase Mode of the Chopper.
    THIS FUNCTION CAN NOT BE USED BY USERS !!!
    param  value:  Type of phase desired (1 = Energy, 3 = TotalDelay)
    '''
    cmds.append(Set('BL5:Chop:Skf1:PhaseEntryMode', value, completion = True))
    cmds.append(Set('BL5:Chop:Skf2:PhaseEntryMode', value, completion = True))
    cmds.append(Set('BL5:Chop:Skf3:PhaseEntryMode', value, completion = True))
    cmds.append(Set('BL5:Chop:Skf4:PhaseEntryMode', value, completion = True))
    cmds.append(Set('BL5:Chop:Skf5:PhaseEntryMode', value, completion = True))
    if value == 1:
       print(str(datetime.datetime.now()) + " ...Changing Phase Mode to Energy.")
       cmds.append(Set('BL5:CS:LineLog:Add',"Changed Phase Mode to Energy."))
    else:
       print(str(datetime.datetime.now()) + " ...Changing Phase Mode to Total Delay.")
       cmds.append(Set('BL5:CS:LineLog:Add',"Changed Phase Mode to Total Delay."))

def New_total_delay(mus1,mus2,mus3,mus4,mus5):
    '''
    This Function Changes the total delay of the Choppers in us.
    THIS FUNCTION CAN NOT BE USED BY USERS !!!
    param  mus1:  Type the new delay for Chop1 in us.
    param  mus2:  Type the new delay for Chop2 in us.
    param  mus3:  Type the new delay for Chop3 in us.
    param  mus4:  Type the new delay for Chop4 in us.
    param  mus5:  Type the new delay for Chop5 in us.
    '''
    cmds.append(Set('BL5:Chop:Skf1:TotalDelaySet', mus1, completion = True))
    cmds.append(Set('BL5:Chop:Skf2:TotalDelaySet', mus2, completion = True))
    cmds.append(Set('BL5:Chop:Skf3:TotalDelaySet', mus3, completion = True))
    cmds.append(Set('BL5:Chop:Skf4:TotalDelaySet', mus4, completion = True))
    cmds.append(Set('BL5:Chop:Skf5:TotalDelaySet', mus5, completion = True))

def CrossCHK_Chopper(mode=0):
    '''
    This Function Checks our the status of the Choppers. 
    THIS FUNCTION CAN NOT BE USED BY USERS !!!
    '''
    var1=caget('BL5:Chop:Skf1:PhaseEntryMode')
    var2=caget('BL5:Chop:Skf2:PhaseEntryMode')
    var3=caget('BL5:Chop:Skf3:PhaseEntryMode')
    var4=caget('BL5:Chop:Skf4:PhaseEntryMode')
    var5=caget('BL5:Chop:Skf5:PhaseEntryMode')
    if (mode == 0):
       if (var1*var1+var2*var2+var3*var3+var4*var4+var5*var5) == 5:
          print("CNCS Choppers are Ready to collect data.")
       else:
          print("CNCS Choppers are NOT Ready to collect data, please check the Phase entry.")
          print("Ch1: {0}, Ch2: {1}, Ch3: {2}, Ch4: {3}, Ch5: {4}".format(var1,var2,var3,var4,var5))
    else:
       caput('BL5:Chop:Skf1:PhaseEntryMode',mode)
       caput('BL5:Chop:Skf2:PhaseEntryMode',mode)
       caput('BL5:Chop:Skf3:PhaseEntryMode',mode)
       caput('BL5:Chop:Skf4:PhaseEntryMode',mode)
       caput('BL5:Chop:Skf5:PhaseEntryMode',mode)
...
def set_temp(args, value, args2='none', value2=0.0, waitime=5.0):
    '''
    ...
    THE FIRST ARGS MUST be the slowest PV to reach that temperature, for mag-005 is always fitssm.
    ...
    print("Time-out to reach the desired temperature: {0} hours or {1} seconds".format(waitime,waitsecond))
    if args not in __Aliases_PVs.keys() or ((args2.lower() != 'none') and (args2 not in __Aliases_PVs.keys())):
       ...
       if args.lower() == 'fitssm':
          cmds.append(Set(__Aliases_PVs[args], value, readback='BL5:SE:SampleTemp', tolerance=temptol, timeout=waitsecond))
       else:
       ...
       if args.lower() == 'fitssm':
          cmds.append(Set(__Aliases_PVs[args2], value2, completion = False))
          cmds.append(Set(__Aliases_PVs[args], value, readback='BL5:SE:SampleTemp', tolerance=temptol, timeout=waitsecond))
       else:
...
def set_field(args,value,ramping='True'):
    ...
    param   ramping:     Set the mode for collecting data: True waits for the persistent field, 
                         False allows users to collect the data while current is ramping. Default is True !!!
    '''
    ...
    if ramping == 'True':
       if finalfield <= 5.5:
          ...
          delay(timefield)
       elif finalfield > 5.5 and finalfield <= 7.0:
          timefield= (value/0.025)*(value*0.8) + 300.0 + (currentfield/0.025)
          delay(timefield)
       elif finalfield > 7.0:
          timefield= (value/0.01)*(value*0.55) + 200.0 + (currentfield/0.01)
          delay(timefield)
    else:
       print("Collecting data while ramping the current.")
       cmds.append(Set('BL5:CS:LineLog:Add',"Collecting data while ramping the current"))
       if value <= 5.5:
          timefield= (value/0.1)*(value*0.8) + 270.0 + (currentfield/0.1)
...
def UBset(args):
    '''
    This command sets the UB matrix of a sample in the Nexus File.
    param  args:  Array or list containing the 9 entries of the UB matrix.
    '''
    if isinstance(args,np.ndarray) == True:
       UBmatrix=args.__repr__()
    elif isinstance(args,list) == True:  
       arargs=np.array(args)
       UBmatrix=arargs.__repr__()
    else:  
       raise RuntimeError("Argument is not a valid list or an array.")
    cmds.append(Set('BL5:CS:CrystalAlign:UBMatrix',UBmatrix))
    cmds.append(Set('BL5:CS:LineLog:Add',"Set new UB matrix {0}.".format(args)))
    print("Set new UB matrix {0}".format(args))
...
def cmdUBset(args):
    ...
    if isinstance(args,np.ndarray) == True:
       ...
       raise RuntimeError("Argument is not a valid list or an array.")
    caput('BL5:CS:CrystalAlign:UBMatrix',UBmatrix)
    caput('BL5:CS:LineLog:Add',"Set new UB matrix {0}.".format(args))
    print("Set new UB matrix {0}".format(args))
...
def br(h,k,l,pv,scattering='left',wait=True):
    '''
    This function drives a motor to the (h,k,l) peak specified, the peak can be observed.
    either on the 'left' or 'right' side of the detector array.
    param   h,k,l:         Selected Miller Indices of the peak.
    ...
    if pv not in __Aliases_dict.keys():
       ...
       SROIxy(np.rad2deg(theta)-2.5,np.rad2deg(theta)+2.5,phi-5.0,phi+5.0)
       __Aliases_dict[pv](omega)
       cmds.append(Set('BL5:CS:LineLog:Add',"Driving to: ({0},{1},{2}) at Omega={3}".format(h,k,l,omega)))
       print("Bragg to h={0} k={1} l={2}".format(h,k,l))
...
def SROIxy(tthmin,tthmax,phimin,phimax):
    ...
    config_path = "/home/bl-user/Script_Test/CNCS_config/CNCS_Deg_pixel.txt" 
    filec=np.loadtxt(config_path,delimiter=",")
    pixels=np.arange(0,len(filec))
    pixelcenter=[]
    tthcenter=(tthmin+tthmax)/2.0
    phicenter=(phimin+phimax)/2.0
    for x in pixels:
      if ((tthcenter-0.5) <= filec[x][0] <= (tthcenter+0.5)) and ((phicenter-0.33) <= filec[x][1] <= (phicenter+0.33)):
         pixelcenter.append(np.array([filec[x][2],filec[x][3]])) 
    pixel1=pixelcenter[0][0]-np.abs(tthcenter-tthmin)/0.45
    pixel2=pixelcenter[0][1]-np.abs(phicenter-phimin)/0.33
    tthsize=np.abs(tthmax-tthmin)/0.45
    phisize=np.abs(phimax-phimin)/0.33
    if pixel1 <= 0.0:
      pixel1=1.0
    elif pixel1 > 422.0:
      pixel1=422.0
    if pixel2 <= 0.0:
      ...
      pixel2=134.0
    if (pixel1+tthsize) > 422.0:
      tthsize=(422.0-pixel1)
    if (pixel2+phisize) > 134.0:
      phisize=(134.0-pixel2)    
    caput('BL5:Det:N1:Det1:XY:ROI:1:MinX',int(np.round(pixel1)))
    caput('BL5:Det:N1:Det1:XY:ROI:1:SizeX',int(np.round(tthsize)))
    caput('BL5:Det:N1:Det1:XY:ROI:1:MinY',int(np.round(pixel2)))
...
def BROIxy(tthmin,tthmax,phimin,phimax):
    ...
    if (pixel2+phisize) > 134.0:
      phisize=(134.0-pixel2)    
    caput('BL5:Det:N1:Det1:XY:ROI:2:MinX',int(np.round(pixel1)))
    caput('BL5:Det:N1:Det1:XY:ROI:2:SizeX',int(np.round(tthsize)))
    caput('BL5:Det:N1:Det1:XY:ROI:2:MinY',int(np.round(pixel2)))
...
__Min_PVs={'samplez':'BL5:Mot:sampleZ.LLM','beaml':'BL5:Mot:beamstopL.LLM','beamr':'BL5:Mot:beamstopR.LLM','huber':'BL5:Mot:huber.LLM','axis1':'BL5:Mot:Sample:Axis1.LLM','vstick':'BL5:Mot:Sample:Axis1.LLM','3sample':'BL5:Mot:Sample:Axis1.LLM',
    ...
    'vtifitssm':'BL5:SE:Lakeshore:CALC_IN_WINDOW1.A','fitssm':'BL5:SE:Lakeshore:CALC_IN_WINDOW2.A', 'dilfridge_sample':'BL5:SE:Dilfridge:Temperature5.HIHI'}
...
__Max_PVs={'samplez':'BL5:Mot:sampleZ.HLM','beaml':'BL5:Mot:beamstopL.HLM','beamr':'BL5:Mot:beamstopR.HLM','huber':'BL5:Mot:huber.HLM','axis1':'BL5:Mot:Sample:Axis1.HLM','vstick':'BL5:Mot:Sample:Axis1.HLM','3sample':'BL5:Mot:Sample:Axis1.HLM',
    ...
    'vtifitssm':'BL5:SE:Lakeshore:CALC_IN_WINDOW1.A','fitssm':'BL5:SE:Lakeshore:CALC_IN_WINDOW2.A', 'dilfridge_sample':'BL5:SE:Dilfridge:Temperature5.LOLO'}
...
__Aliases_PVs={'samplez':'BL5:Mot:sampleZ','beaml':'BL5:Mot:beamstopL','beamr':'BL5:Mot:beamstopR','huber':'BL5:Mot:huber','ei':'BL5:Chop:Gbl:EnergyReq','wl':'BL5:Chop:Skf1:WavelengthUserReq',
    ...
    'ccr10s2':'BL5:SE:Lakeshore:SETP_S2','vticryo6':'BL5:SE:Cryo:Temp:SETP_S1','cryo6':'BL5:SE:Cryo:Temp:SETP_S2','vtifitssm':'BL5:SE:FitSam:OP1SP','fitssm':'BL5:SE:FitSam:SampleTempSP', 'dilfridge_sample':'BL5:SE:Dilfridge:WriteTemp'}
