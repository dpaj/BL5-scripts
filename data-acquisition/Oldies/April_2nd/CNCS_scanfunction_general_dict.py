...
def br(h,k,l,args='left', wait=True):
    '''
...
def runexp(title, energy, name, pos, args, value): 
    ...
    if name not in __Aliases_PVs.keys():
       print("\n PV '{0}' not found !!!".format(name))
       SpellChk(name)
       print("\n")
       raise RuntimeError("Not a valid PV, Check Spelling.")
    elif args not in __Aliases_PVs.keys():
       print("\n PV '{0}' not found !!!".format(args))
       SpellChk(args)
       print("\n")
       raise RuntimeError("Not a valid PV, Check Spelling.")
    else:
...
def SROIxy(tthmin,tthmax,phimin,phimax):
    ...
    tthpixel=[292-(2.236*abs(3.8-tthmin)),292-(2.236*abs(3.8-tthmax))]
    tthsize=2.236*abs(tthmax-tthmin)
    phipixel=[67.0-(1.516*phimin),67.0-(1.516*phimax)]
    phisize=1.516*abs(phimax-phimin)    
...
__Aliases_dict={'samplez':movesampleZ,'beaml':beamstopL,'beamr':beamstopR,'huber':movehuber,'ei':ei,'wl':WL,
...
__Aliases_cmddict={'samplez':cmdmovesampleZ,'beaml':cmdbeamstopL,'beamr':cmdbeamstopR,'huber':cmdmovehuber,'collimator':collimator,'ei':cmdei,'wl':cmdWL,
...
__Aliases_PVs={'samplez':'BL5:Mot:sampleZ','beaml':'BL5:Mot:beamstopL','beamr':'BL5:Mot:beamstopR','huber':'BL5:Mot:huber','ei':'BL5:Chop:Gbl:EnergyReq','wl':'BL5:Chop:Skf1:WavelengthUserReq',
    ...
    'axis2':'BL5:Mot:Sample:Axis2','title':'BL5:SMS:RunInfo:RunTitle','comment':'BL5:SMS:Marker:NotesComment','pc':'BL5:Det:PCharge:C','time':'BL5:CS:RunControl:RunTimer'}
