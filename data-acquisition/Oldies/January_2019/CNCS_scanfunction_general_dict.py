...
def set_field(args,value,ramping='True'):
    ...
    if ramping == 'True':
       if finalfield <= 5.5:
          ...
          timefield= (value/0.01)*(value*0.55) + 180.0
          delay(timefield)
    else:
...
def SROIxy(tthmin,tthmax,phimin,phimax):
    ...
    pixelmax=[]
    for x in pixels:
      if ((tthmin-0.5) <= filec[x][0] <= (tthmin+0.5)) and ((phimin-0.5) <= filec[x][1] <= (phimin+0.5)):
         pixelmin.append(np.array([filec[x][2],filec[x][3]]))      
    for x in pixels:
      if ((tthmax-0.5) <= filec[x][0] <= (tthmax+0.5)) and ((phimax-0.5) <= filec[x][1] <= (phimax+0.5)):
         pixelmax.append(np.array([filec[x][2],filec[x][3]]))
    pixel1=np.array([pixelmax[0][0],pixelmin[0][0]])
    pixel2=np.array([pixelmax[0][1],pixelmin[0][1]])
    pixel1=sorted(pixel1)
    pixel2=sorted(pixel2)
    tthsize=np.abs(pixelmin[0][0]-pixelmax[0][0])
    phisize=np.abs(pixelmin[0][1]-pixelmax[0][1])     
    caput('BL5:Det:N1:Det1:XY:ROI:1:MinX',int(np.round(pixel1[0])))
    caput('BL5:Det:N1:Det1:XY:ROI:1:SizeX',int(np.round(tthsize)))
    caput('BL5:Det:N1:Det1:XY:ROI:1:MinY',int(np.round(pixel2[0])))
...
def BROIxy(tthmin,tthmax,phimin,phimax):
    ...
    tthsize=np.abs(pixelmin[0][0]-pixelmax[0][0])
    phisize=np.abs(pixelmin[0][1]-pixelmax[0][1])
    caput('BL5:Det:N1:Det1:XY:ROI:2:MinX',int(np.round(pixel1[0])))
    caput('BL5:Det:N1:Det1:XY:ROI:2:SizeX',int(np.round(tthsize)))
    caput('BL5:Det:N1:Det1:XY:ROI:2:MinY',int(np.round(pixel2[0])))
